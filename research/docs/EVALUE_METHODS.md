...
[`tests/test_evalue.py`](../../tests/test_evalue.py) (the standalone e-process, 25 tests)
and [`tests/test_evalue_ledger.py`](../../tests/test_evalue_ledger.py) (the ledger wiring,
39 tests — see [§"How the ledger integrates it"](#how-the-ledger-integrates-it-wi-29-wired-2026-07-09) below).
...
4. **The sidecar is a derived inspection snapshot, not an authority (WI-44/45).** It is
   regenerated on every run and never read back. Every run verifies the hash
   ...
   Historical note (past design, no longer in the code) — the pre-WI-44 runner resumed from
   the sidecar: Each run persists
   ...
   observation in exactly once, so the registered-mode per-hypothesis guarantee carries
   across repeated command invocations under the header note's assumptions (legacy lazy
   mode remains exploratory). If
