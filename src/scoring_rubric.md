...
**Boundary.** This file defines the *released reference rubric*. The production engine scores
against the same anchors but its exact factor elaborations, domain-specific adjustments, and dim
weighting are private; where this file must choose a reference convention (marked *reference
definition*), the convention is normative for public use and an approximation of the private
engine. Scores are assigned at the EVALUATE stage (bear-case-first), re-judged at ADJUDICATE for
...
**Threshold provenance.** The 0.22 / 0.10 tier cuts are operational constants inherited from the
build-out configuration, set before the forward window opened; they were not fitted to outcomes
(none existed yet). Whether they are well-calibrated is itself a declared forward question — the
December read-out reports realized outcomes by admitted tier.

## Factor anchors

All factors live in [0, 1]. Interpolate between anchor rows; **never default a factor you cannot
assess** (see Missing data below). Evidence tiers apply: an anchor claimed from tertiary sources
alone is not established (source-tier rule).
...
| Dim | 0.9 — binding | 0.5 — contestable | 0.2 — soft |
|---|---|---|---|
| `physical_indispensability` | no known process route without this step/input | substitutable at material (2–3×) cost or redesign | convenience input; routes around it exist |
| `substitutability`⁻¹ | no qualified alternative supplier/process; qualification barrier documented (certification, change-control) | alternatives exist; switching costs real but bounded | commodity — many interchangeable sources |
| `capacity_lead_time` | ≥3 years greenfield (permits, certification, construction) | 1–2 years | <1 year, or idle capacity restartable |
| `supplier_concentration` | 1–2 suppliers hold ≥70% share | top-3 hold ~50% | fragmented |
| `pricing_power` | realized price escalation visible in filings/contracts | mixed/episodic | price-taker |
...
[synthetic walkthrough](../examples/synthetic_walkthrough/) lands at 0.211 vs the 0.22 core cut
precisely to make this concrete.
...
- Two-reader check (the audit protocol this rubric is built for): both readers score
  independently from the card and its evidence; any factor differing by more than 0.1 is
  *unsettled* → watchlist until a dated source settles it. Disagreement is measured, not averaged
  away.
